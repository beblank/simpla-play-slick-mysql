...
class Users @Inject() (dbConfigProvider:DatabaseConfigProvider){
    def dbConfig: DatabaseConfig[JdbcProfile] = dbConfigProvider.get[JdbcProfile]

    val users = TableQuery[UserTableDef]

    def add(user:User):Future[String] = {
        dbConfig.db.run(users += user).map( res => "User successful added").recover{
    ...
    def listAll: Future[Seq[User]] = dbConfig.db.run(users.result)
}